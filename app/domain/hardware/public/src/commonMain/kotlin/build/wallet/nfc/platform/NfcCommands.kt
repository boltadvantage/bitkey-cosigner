...
/**
 * Shared NFC commands supported by all hardware types (W1 and W3).
 *
 * Each method takes an [NfcSession] as well as their respective arguments.
 * For W3-only commands, see [W3NfcCommands].
 */
...
/**
 * W3-only NFC commands that require W3 hardware.
 *
 * These are confirmable or composite commands only supported by W3 firmware.
 * Extends [NfcCommands] so a [W3NfcCommands] instance can be used anywhere
 * a [NfcCommands] is expected.
 */
@Suppress("TooManyFunctions")
interface W3NfcCommands : NfcCommands {
  /**
   ...
   *
   * This is a confirmable command that shows "Confirm this is your account?"
   ...
   *
   * On W1, callers should use [unsealSymmetricKey] directly.
   *
   * @param session the active NFC session
...
/**
 * Commands wrapper that can resolve the initially detected device info for the current NFC session.
 *
 * This is used by the auto-detect routing layer so interceptors and call sites can reason about the
 * tapped hardware without forcing additional live [getDeviceInfo] APDUs once the identity is known.
 */
interface HardwareIdentityAwareNfcCommands {
  suspend fun resolvedDeviceInfo(session: NfcSession): FirmwareDeviceInfo
}

/**
 * Requires that this [NfcCommands] is actually a [W3NfcCommands].
 *
 * Use this at call sites that need W3-only commands. The cast is safe when the
 * [NfcCommandsProvider] resolved to a W3 implementation for W3 hardware.
 *
 * @throws NfcException.CommandError if the commands are not W3
 */
fun NfcCommands.requireW3(): W3NfcCommands =
  this as? W3NfcCommands
    ?: throw NfcException.CommandError(
      message = "W3 commands required but got ${this::class.simpleName}"
    )

/**
 * Returns the initially detected device info for the tapped hardware.
 *
 * Auto-detecting command wrappers may satisfy this from session-scoped cached identity.
 * Other commands fall back to a live [getDeviceInfo] read.
 */
suspend fun NfcCommands.detectedDeviceInfo(session: NfcSession): FirmwareDeviceInfo =
  (this as? HardwareIdentityAwareNfcCommands)?.resolvedDeviceInfo(session)
    ?: getDeviceInfo(session)

/**
 * Returns the actual hardware type for the tapped device.
 *
 * Auto-detecting command wrappers derive this from the resolved device info. Raw W3 and W1 command
 * implementations fall back to their static implementation type.
 */
suspend fun NfcCommands.actualHardwareType(session: NfcSession): HardwareType =
  when (this) {
    is HardwareIdentityAwareNfcCommands -> detectedDeviceInfo(session).hardwareType()
    is W3NfcCommands -> HardwareType.W3
    else -> HardwareType.W1
  }

/**
 * Requires that the tapped hardware is actually W3 for this session.
 *
 * This is preferred over [requireW3] at W3-only entry points because it surfaces
 * [NfcException.WrongHardwareType] when auto-detect resolves to W1.
 */
suspend fun NfcCommands.requireW3(session: NfcSession): W3NfcCommands {
  val actualType = actualHardwareType(session)
  if (actualType != HardwareType.W3) {
...
suspend fun W3NfcCommands.lostAppRecoverySignChallenge(
  session: NfcSession,
  challenge: String,
